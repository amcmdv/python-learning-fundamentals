...
## Step 3 — Add Root Configuration Files and Install Git Hook
...
Install the `nbstripout` git hook — this must be done before any notebook is committed. It strips cell outputs automatically on every commit, preventing embedded user paths from entering git history.
...
| **New file — no legacy source** | `02-control-flow/loops_and_conditionals.ipynb` — create in Phase 2 (see `GAP-01` in `BACKLOG.md`) |
...
| `Milestone Project 1/[4. Interaction] Simple User Interaction.ipynb` | Merge `displayBoard` content into `confidence_game.ipynb`; discard source file — content is not substantial enough to warrant a standalone notebook |
...
- `.ipynb_checkpoints/[How] Url Get request-checkpoint.ipynb` → `scratch/url_get_request.ipynb` (incomplete, not in curriculum — see `DEF-01` in `BACKLOG.md`)
- `.ipynb_checkpoints/Error-checkpoint.ipynb` → **discard**. This is an auto-save artifact of a scratch session. The canonical source for the error handling bug fix is `[Short Exercise] Errors and Exceptions.ipynb` → `04-error-handling/exercises.ipynb`. Do not merge from the checkpoint.
---
...
### Fix 6 — `07-projects/project-01-confidence-game/warmup_exercises.ipynb`
 
```python
# BEFORE
from IPython.display import clear_output   # imported as clear_output
 
clearOutput()   # NameError: name 'clearOutput' is not defined
...
## Step 6 — Register the Python 3.11 Kernel
 
All legacy notebooks carry kernel metadata specifying Python 3.8.3. After activating the `py-fundamentals` environment, register the correct kernel so JupyterLab uses the right interpreter for every notebook.
...
# Expected output includes: py-fundamentals  /Users/<user>/Library/Jupyter/kernels/py-fundamentals
 
# Confirm no notebook still references the old kernel version
grep -r '"version": "3.8' --include="*.ipynb" .
# Expected: zero matches (outputs were cleared in Step 1; kernel spec is updated on next open/save)
```
 
> **Note**: Kernel metadata in `.ipynb` files is updated when the notebook is opened and saved in JupyterLab with the correct kernel selected. Open each notebook, select kernel `Python 3.11 (py-fundamentals)` from the kernel picker, and save. The `pytest --nbmake` run in Step 7 will use the registered kernel automatically if invoked with `--nbmake-kernel=py-fundamentals`.
 
---
 
## Step 7 — Validate All Notebooks
...
## Step 8 — Initial Commit to New Repo
...
## Step 9 — Deprecate Legacy Repository
